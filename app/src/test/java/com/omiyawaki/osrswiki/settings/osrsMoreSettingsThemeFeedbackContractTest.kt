...
class osrsMoreSettingsThemeFeedbackContractTest {
    ...
    fun donateBindsStoreLocalizedPricesAndKeepsHonestCopy() {
        ...
        assertTrue(strings.contains("donate_wiki_description\">The Old School RuneScape Wiki is run by volunteers. Support them too if you can."))
        assertFalse(strings.contains("Old School RuneScape Wiki (the web version)"))
        assertFalse(fossStrings.contains("Old School RuneScape Wiki (the web version)"))
        assertFalse(fossStrings.contains("separate volunteers"))
        assertTrue(layout.contains("@string/donate_header"))
        assertTrue(layout.contains("@string/donate_description"))
        val donateTitle = layout.substringAfter("android:id=\"@+id/donate_title\"").substringBefore("android:id=\"@+id/donate_description\"")
        assertTrue(donateTitle.contains("@style/AppTextAppearance.SettingsSection"))
        assertFalse(donateTitle.contains("SettingsPageHero"))
        val donateDescription = layout.substringAfter("android:id=\"@+id/donate_description\"").substringBefore("android:id=\"@+id/amount_chip_group\"")
        assertTrue(donateDescription.contains("android:layout_marginTop=\"16dp\""))
        assertTrue(layout.contains("@string/donate_wiki_description"))
        assertTrue(layout.contains("wiki_support_title"))
        assertTrue(layout.contains("wiki_support_description"))
        assertTrue(layout.contains("app:layout_constraintTop_toBottomOf=\"@id/wiki_support_description\""))
        val wikiDescription = layout.substringAfter("android:id=\"@+id/wiki_support_description\"").substringBefore("android:id=\"@+id/wiki_donate_button\"")
        assertFalse(wikiDescription.contains("android:visibility=\"gone\""))
