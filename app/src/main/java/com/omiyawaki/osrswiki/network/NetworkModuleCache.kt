...
class NetworkModuleCache private constructor(context: Context) {
    ...
    /**
     * Synchronous read for WebView interception. The caller must only use this
     * after deciding that serving cached content is preferable to waiting for
     * the normal WebView network stack.
     */
    fun getCachedResponseIfPresent(url: String): String? {
        return try {
            val cacheKey = generateCacheKey(url)
            val cacheFile = File(cacheDir, cacheKey)
            if (cacheFile.exists()) {
            ...
            Log.w(TAG, "Error reading from cache: ${e.message}")
            null
        }
    }
