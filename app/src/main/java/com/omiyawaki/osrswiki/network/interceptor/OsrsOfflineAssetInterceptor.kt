...
class OsrsOfflineAssetInterceptor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        try {
            ...
            savedFile = fileStorageManager.saveAsset(
            ...
            val localFilePath = savedFile.absolutePath
            ...
            runBlocking {
                var offlineAsset = offlineAssetDao.getAssetByOriginalUrl(assetOriginalUrl)
                ...
                if (offlineAsset == null) {
                    ...
                    offlineAsset.localFilePath = localFilePath // Update path in case it was re-downloaded/moved
                    offlineAsset.downloadTimestamp = currentTime
                    if (updated) {
                        offlineAsset.usedByArticleIds = existingUsedBySet.joinToString(",")
                    }
                    offlineAssetDao.update(offlineAsset)
                    Log.i(TAG, "Updated existing OfflineAsset for $assetOriginalUrl, linked/re-linked to entry $savedArticleEntryId. Used by: ${offlineAsset.usedByArticleIds}")
                }
            }
            ...
            val fileResponseBody = savedFile.readBytes().toResponseBody(responseBody.contentType())
