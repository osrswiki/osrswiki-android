...
class MapFragment : Fragment() {
    ...
    companion object {
        ...
        fun newInstance(mapData: MapDiagnosticData): MapFragment {
            return MapFragment().apply {
                arguments = bundleOf(
                    ARG_LAT to mapData.lat,
                    ARG_LON to mapData.lon,
                    ARG_ZOOM to mapData.zoom,
                    ARG_PLANE to mapData.plane
                )
            }
        }
        ...
        mapView?.getMapAsync { maplibreMap ->
            ...
            maplibreMap.setStyle(Style.Builder().fromJson(styleJson)) { style ->
                ...
                if (gameLon != null && gameLat != null) {
                    // This is an infobox map. Restore the hardcoded zoom of 6.0 to match old behavior.
                    val zoom = 6.0
                    val target = gameToLatLng(gameLon, gameLat)
                    cameraBuilder.target(target).zoom(zoom)
                } else {
                    // This is the main map page. Use the default zoom.
    ...
    private fun setMapBounds(map: MapLibreMap) {
        val north = 85.0511; val west = -180.0; val south = -85.0511; val east = 180.0
        val bounds = LatLngBounds.Builder().include(LatLng(north, west)).include(LatLng(south, east)).build()
        map.setLatLngBoundsForCameraTarget(bounds)
    }

    private fun setupMapLayers(style: Style) {
        for (floor in 0..maxFloor) {
            val sourceId = "osrs-source-$floor"; val layerId = "osrs-layer-$floor"
            val mbtilesFile = File(requireContext().filesDir, mapFiles[floor])
            style.addSource(RasterSource(sourceId, "mbtiles://${mbtilesFile.absolutePath}"))
    ...
    private fun setupFloorControls() {
        ...
        if (maxFloor > 0) {
            ...
            binding.floorControlUp.setOnClickListener { if (currentFloor < maxFloor) showFloor(currentFloor + 1) }
            binding.floorControlDown.setOnClickListener { if (currentFloor > 0) showFloor(currentFloor - 1) }
        } else {
            binding.floorControls.visibility = View.GONE
        }
    }
    ...
    private fun showFloor(newFloor: Int) {
        ...
        for (floor in 0..maxFloor) {
            ...
            layer?.let {
                when {
                    floor == newFloor -> it.setProperties(PropertyFactory.visibility(Property.VISIBLE), PropertyFactory.rasterOpacity(1.0f))
                    floor == 0 && newFloor > 0 -> it.setProperties(PropertyFactory.visibility(Property.VISIBLE), PropertyFactory.rasterOpacity(GROUND_FLOOR_UNDERLAY_OPACITY))
