...
class MapFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mapView = MapView(requireContext()).apply {
            contentDescription = getString(R.string.map_accessibility_description)
        }
        ...
        mapView?.getMapAsync { maplibreMap ->
            ...
            maplibreMap.setStyle(Style.Builder().fromJson(styleJson)) { style ->
                ...
                if (gameLon != null && gameLat != null) {
                    ...
                    cameraBuilder
                        .target(LatLng(osrsMapDefaultView.LATITUDE, osrsMapDefaultView.LONGITUDE))
    ...
    private fun setMapBounds(map: MapLibreMap) {
        ...
        Log.d(logTag, "- LatLng bounds: SW(${southWest.latitude}, ${southWest.longitude}), NE(${northEast.latitude}, ${northEast.longitude})")
    ...
    fun debugStateForTesting(): DebugState {
        return DebugState(
            hasMap = map != null,
    ...
    fun zoomByForTesting(delta: Double): Boolean {
        val mapLibreMap = map ?: return false
        val current = mapLibreMap.cameraPosition
        val newZoom = (current.zoom + delta).coerceIn(0.0, 12.0)
