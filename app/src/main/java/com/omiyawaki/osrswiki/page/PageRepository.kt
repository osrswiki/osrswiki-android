...
class PageRepository(
    ...
    fun getArticleByTitle(title: String, theme: Theme, forceNetwork: Boolean): Flow<Result<PageUiState>> = flow {
        emit(Result.Loading)
        if (!forceNetwork) {
            val offlineArticle = getOfflineArticleByTitle(title)
            if (offlineArticle != null) {
                emit(Result.Success(offlineArticle))
                return@flow
            }
        }
    ...
    /**
     * Get article content that is already available locally, without attempting
     * a remote fetch. This is used for offline link navigation, where an
     * uncached target should produce immediate feedback instead of waiting for
     * a network timeout.
     */
    internal suspend fun getOfflineArticleByTitle(title: String): PageUiState? {
