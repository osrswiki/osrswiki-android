...
class PageLinkHandler(
    ...
    override fun onInternalArticleLinkClicked(articleTitleWithUnderscores: String, fullUri: Uri) {
        ...
        coroutineScope.launch {
            if (!isNetworkAvailable()) {
                val offlineTargetUiState = withContext(Dispatchers.IO) {
                    pageRepository.getOfflineArticleByTitle(targetPageTitleWithSpaces)
                }
    ...
    private fun showOfflineTargetUnavailable(targetPageTitle: String) {
        Toast.makeText(
            context,
