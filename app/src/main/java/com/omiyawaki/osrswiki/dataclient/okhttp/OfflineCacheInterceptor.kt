...
internal class OfflineCacheInterceptor(
    ...
    private fun saveResponse(request: Request, response: Response, saveHeaderValue: String): Response {
        ...
        Log.d("OSRSWIKI_INTERCEPTOR", "[D] Saved metadata for $url to ${metadataFile.absolutePath}")

        OfflineResponseFileWriter.copyToFile(responseBody.byteStream(), contentFile)
        Log.d("OSRSWIKI_INTERCEPTOR", "[D] Saved content for $url to ${contentFile.absolutePath}")
        ...
        appDatabase.runInTransaction {
            ...
            Log.i("OSRSWIKI_INTERCEPTOR", "[I] Offline object metadata saved to DB for $url with ID: $insertedOfflineObjectId")
            ...
            if (saveType == OfflineObject.SAVE_TYPE_READING_LIST && pageLibIdsStr.isNotBlank()) {
                ...
                if (pageIds.isNotEmpty()) {
                    ...
                    pageIds.forEach { pageId ->
                        try {
                            readingListPageDao.updatePageStatusToSavedAndMtimeBlocking(
                                pageId,
                                ReadingListPage.STATUS_SAVED,
                                currentTime
                            )
                            Log.d("OSRSWIKI_INTERCEPTOR", "[D] Updated status for ReadingListPage ID $pageId to SAVED.")
                        } catch (e: Exception) {
                            Log.e("OSRSWIKI_INTERCEPTOR", "[E] Error updating status for ReadingListPage ID $pageId in transaction", e)
                            throw e
                        }
                    }
                }
        ...
        return response.newBuilder()
            .body(contentFile.source().buffer().asResponseBody(responseBody.contentType(), contentFile.length()))
            .build()
    }
