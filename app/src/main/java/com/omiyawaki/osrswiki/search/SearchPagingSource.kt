package com.omiyawaki.osrswiki.search

import android.util.Log
import androidx.paging.PagingSource
import androidx.paging.PagingState
import com.omiyawaki.osrswiki.database.ArticleMetaDao
import com.omiyawaki.osrswiki.network.SearchResult
import com.omiyawaki.osrswiki.network.WikiApiService
import com.omiyawaki.osrswiki.page.osrsWikiWebViewUrl
import kotlinx.coroutines.async
import kotlinx.coroutines.coroutineScope
import retrofit2.HttpException
import java.io.IOException
import java.util.concurrent.CancellationException

private const val OSRS_WIKI_STARTING_PAGE_OFFSET = 0
private const val THUMBNAIL_SIZE = 240

class SearchPagingSource(
    private val apiService: WikiApiService,
    private val query: String,
    private val articleMetaDao: ArticleMetaDao
) : PagingSource<Int, SearchResult>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SearchResult> {
        val currentOffset = params.key ?: OSRS_WIKI_STARTING_PAGE_OFFSET
        val limit = params.loadSize

        if (query.isBlank()) {
            return LoadResult.Page(data = emptyList(), prevKey = null, nextKey = null)
        }

        return try {
            // Fulltext Cirrus misses title-prefix hits the website search box
            // returns (for example "earth ru" → Earth rune). Fetch prefixsearch
            // in parallel on the first page, then rerank the union.
            val apiQuery = SearchQueryPolicy.networkQuery(query)
            val (response, prefixPages, openSearchPages) = coroutineScope {
                val fulltext = async {
                    apiService.generatedPrefixSearch(
                        query = apiQuery,
                        limit = limit,
                        offset = currentOffset,
                        thumbSize = THUMBNAIL_SIZE
                    )
                }
                val prefix = async {
                    if (currentOffset == OSRS_WIKI_STARTING_PAGE_OFFSET) {
                        runCatching {
                            apiService.generatedTitlePrefixSearch(
                                query = SearchQueryPolicy.apiQuery(query),
                                limit = minOf(limit, 10),
                                thumbSize = THUMBNAIL_SIZE
                            ).query?.pages.orEmpty()
                        }.getOrDefault(emptyList())
                    } else {
                        emptyList()
                    }
                }
                val openSearch = async {
                    if (currentOffset == OSRS_WIKI_STARTING_PAGE_OFFSET) {
                        runCatching {
                            osrsOpenSearchParser.parse(
                                apiService.openSearch(SearchQueryPolicy.apiQuery(query), 10).bytes()
                            )
                        }.getOrDefault(emptyList())
                    } else {
                        emptyList()
                    }
                }
                Triple(fulltext.await(), prefix.await(), openSearch.await())
            }

            val searchResults = SearchQueryPolicy.merge(
                query,
                prefixPages,
                response.query?.pages.orEmpty()
            ).filter { osrsWikiWebViewUrl.isIncludedInDefaultSearch(it.title) }.map { result ->
                val preview = result.snippet?.trim().orEmpty()
                if (preview.isNotEmpty()) {
                    result.copy(snippet = preview)
                } else {
                    val openSnippet = openSearchPages.firstOrNull {
                        it.title.equals(result.title, ignoreCase = true) && !it.snippet.isNullOrBlank()
                    }?.snippet
                    result.copy(snippet = openSnippet)
                }
            }

            if (searchResults.isEmpty()) {
                return LoadResult.Page(data = emptyList(), prevKey = null, nextKey = null)
            }

            // Enhance results with offline status from the local database.
            val pageIds = searchResults.map { it.pageid }
            val offlineMetas = articleMetaDao.getMetasByPageIds(pageIds)
            val offlinePageIds = offlineMetas.map { it.pageId }.toSet()

            val enhancedSearchResults = searchResults.map { searchResult ->
                val isOffline = offlinePageIds.contains(searchResult.pageid)
                // Use .copy() to create a new instance with the offline status set.
                searchResult.copy(isOfflineAvailable = isOffline)
            }

            val nextKey = response.continuation?.gsroffset
            val prevKey = if (currentOffset == OSRS_WIKI_STARTING_PAGE_OFFSET) null else (currentOffset - params.loadSize).coerceAtLeast(0)

            LoadResult.Page(
                data = enhancedSearchResults,
                prevKey = prevKey,
                nextKey = nextKey
            )
        } catch (exception: IOException) {
            Log.e("SearchPagingSource", "IOException during search", exception)
            return LoadResult.Error(exception)
        } catch (exception: HttpException) {
            Log.e("SearchPagingSource", "HttpException during search", exception)
            return LoadResult.Error(exception)
        } catch (exception: CancellationException) {
            // This is expected when a new search supersedes the current one.
            throw exception
        } catch (exception: Exception) {
            Log.e("SearchPagingSource", "Generic Exception during search", exception)
            return LoadResult.Error(exception)
        }
    }

    override fun getRefreshKey(state: PagingState<Int, SearchResult>): Int? {
        return state.anchorPosition?.let { anchorPosition ->
            val anchorPage = state.closestPageToPosition(anchorPosition)
            anchorPage?.prevKey?.plus(state.config.pageSize) ?: anchorPage?.nextKey?.minus(state.config.pageSize)
        }
    }
}
